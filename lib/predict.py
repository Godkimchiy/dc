...
from lib.yolov5.utils.datasets import LoadImages, LoadStreams
from lib.yolov5.utils.general import check_img_size, non_max_suppression, scale_coords, check_imshow, xyxy2xywh
from lib.yolov5.utils.torch_utils import select_device, time_sync
from lib.yolov5.utils.plots import Annotator, colors
from lib.deep_sort_pytorch.utils.parser import get_config
from lib.deep_sort_pytorch.deep_sort import DeepSort
# import argparse
import os
import platform
import shutil
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)): 
                        ...
                        # todo : remove box-linei (red?)
                        crop_img = im0s[bbox_top:bbox_top+bbox_h, bbox_left:bbox_left+bbox_w]

                        if id in ft_dict:
                            ft_dict[id].append(crop_img)
                        else:
                            ft_dict[id] = [crop_img]
                        
                        if save_jpg:
                            vn_dir = os.path.join(out, '{:d}'.format(video_num))
                            pid_dir = os.path.join(vn_dir, '{:d}'.format(id))
                            fid_path = os.path.join(pid_dir, '{:04d}_{:04d}.jpg'.format(id, frame_idx))
...
class func_task3:
    def __init__(self, args):
        ...
        self.set_num = args.set_num # set_num01
        self.input_paths = [os.path.join(args.dataset_dir, args.set_num, f'set01_drone0{i+1}.mp4') for i in range(3)]
        ...
        with open ('conf/task3.yaml') as f:
        ...
        # etc
        self.reid_ff_path = os.path.join(args.root_dir, f'lib/reid/references/ff.pickle')
    ...
    def reid(self, inputs):
        ...
        for j in range(3):
            for i in inputs[j]:
                ids.append(i)
                reid_ft.append(FeatureExtractor(inputs[j][i]))
        ...
        for i in range(n):
            ...
            if ff_similarity < ff_rm_thresh:
                rm_ids.append(i) 
                continue
            for k in range(n-1):
                if k in rm_ids:
                    continue
                if dist_list[i][k] <= reid_thresh:
                    new_ids[i].append(k)
                    if i != k:
                        rm_ids.append(k) 

        print(" id groups: {0} \n remove: {1}".format(new_ids, rm_ids))
        tmp = list(set(rm_ids))
    ...
    def run(self):
        ...
        # todo : (11/1, with juyeon) pose estimation (moving or not) with args-pose
